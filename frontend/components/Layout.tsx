...
import { 
  ...
  BrainCircuit, Server, LogOut, ChevronRight, X, HeartPulse 
} from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, onLogout }) => {
  ...
  const handleNavClick = (tabId: string) => {
    onTabChange(tabId);
    setIsMobileMenuOpen(false); // Close mobile drawer on click
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      {isMobileMenuOpen && (
        <div 
          className="fixed inset-0 z-30 bg-slate-900/50 backdrop-blur-sm lg:hidden animate-in fade-in"
          onClick={() => setIsMobileMenuOpen(false)}
        />
      )}

      {/* --- SIDEBAR (Hybrid: Mobile Drawer + Desktop Collapsible) --- */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 flex flex-col bg-slate-900 text-white shadow-2xl transition-all duration-300 ease-in-out
      ...
      `}>
        ...
        <div className={`flex items-center h-20 border-b border-slate-800 transition-all ${isSidebarCollapsed ? 'justify-center px-0' : 'justify-between px-5'}`}>
          ...
          <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-white rounded-lg">
            <X className="h-6 w-6" />
          </button>

          {/* Desktop: Collapse Button (Hamburger) - Only visible when expanded */}
          {!isSidebarCollapsed && (
            <button onClick={() => setIsSidebarCollapsed(true)} className="hidden lg:block p-1.5 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors">
        ...
        {isSidebarCollapsed && (
            <div className="hidden lg:flex justify-center py-4 border-b border-slate-800">
                <button onClick={() => setIsSidebarCollapsed(false)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors">
        ...
        <div className="flex-1 px-3 py-6 overflow-y-auto overflow-x-hidden custom-scrollbar">
            ...
            {navItems.map((item) => (
                <button 
                  ...
                  title={isSidebarCollapsed ? item.label : ''} // Tooltip on collapse
                  className={`
                    flex items-center w-full gap-3 px-3 py-3 text-sm font-medium rounded-xl transition-all duration-200 group relative
                    ${activeTab === item.id ? 'bg-teal-600 text-white shadow-lg shadow-teal-900/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}
                ...
                >
                  ...
                  <span className={`truncate transition-all duration-200 ${isSidebarCollapsed ? 'lg:hidden w-0 opacity-0' : 'w-auto opacity-100'}`}>
                    {item.label}
                  </span>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          <div className={`flex items-center gap-3 p-2 rounded-xl hover:bg-slate-800 transition-colors cursor-default group ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold shadow-md ring-2 ring-slate-900 group-hover:ring-slate-700 transition-all flex-shrink-0">
                {user ? getInitials(user.full_name) : 'OP'}
            </div>
            ...
            <button 
                ...
                className={`p-1.5 hover:bg-slate-700 rounded-lg text-slate-500 hover:text-rose-400 transition-colors ${isSidebarCollapsed ? 'hidden' : 'block'}`} 
                title="Log Out"
            >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative w-full bg-slate-50 transition-all duration-300">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm lg:shadow-none">
            <div className="flex items-center gap-4">
                {/* Mobile Menu Trigger */}
                <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-lg active:scale-95 transition-all">
                    <Menu className="h-6 w-6" />
                </button>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg sm:text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                        <ChevronRight className="hidden sm:block h-5 w-5 text-slate-300" />
                        {getActiveLabel()}
                    </h2>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 sm:gap-6 text-xs font-medium text-slate-500">
                ...
                <span className="border-l border-slate-300 pl-4 sm:pl-6 h-4 flex items-center">
                   <span className="hidden sm:inline mr-1">Env:</span> <span className="font-mono font-bold text-slate-700">v{APP_VERSION}</span>
                </span>
            </div>
        </header>
